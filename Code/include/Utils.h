...
public:
    ...
    static void testAndVisit(queue<Vertex*> &q, Edge *e, Vertex *v, double residual);

    /// @brief Function to find an Augmenting Path using BFS
    static bool findAugmentingPath(Graph *g, Vertex *source, Vertex *sink);

    /// @brief Function to find the Minimum Residual Capacity along the Augmenting Path
    static double findMinResidualAlongPath(Vertex *source, Vertex *sink);

    /// @brief Function to Augment Flow along the Augmenting Path with the given Flow Value
    static void augmentFlowAlongPath(Vertex *source, Vertex *sink, double f);

    /// @brief Function implementing the Edmonds-Karp Algorithm
    static void edmondsKarp (Graph *g, string source, string target);
